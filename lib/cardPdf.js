...
const CARD_H_MM = 54;
const W = CARD_W_MM * MM_TO_PT;
const H = CARD_H_MM * MM_TO_PT;
const MARGIN = 8.5;
const GAP = 14;
const PAGE_W = W * 2 + GAP;
const CARD_RADIUS = 6.8;
...
const FRONT = {
  headerH: 36.5, badgeGap: 2.5, badgeH: 8.8, bodyGap: 4,
  photoW: 32.8, photoH: 40, fieldsGap: 6,
  labelW: 39, labelSize: 4.3, valueSize: 5.4, valueLineH: 6.4, rowGap: 1.6,
  sectionGap: 3,
  signCapW: 36.5, signImgSize: 16, capSize: 3, capLineH: 3.6, capGap: 1.5,
  verifySize: 4.8, verifyLineH: 5.7, signColGap: 6.5,
  waveH: 4.6, footerH: 10.5
};
FRONT.fieldsX = MARGIN + FRONT.photoW + FRONT.fieldsGap;
FRONT.valueX = FRONT.fieldsX + FRONT.labelW + 3;
FRONT.fieldsW = W - MARGIN - FRONT.fieldsX;
FRONT.valueW = W - MARGIN - FRONT.valueX;
...
const BACK = {
  headerH: 28, sectionGap: 5.5,
  graphicWRatio: 0.42,
  benefitIcon: 8, benefitGlyph: 4.6, benefitSize: 3.9, benefitLineH: 5.1, benefitGap: 2.2,
  qrBox: 28.5, qrKnSize: 3.8, qrKnLineH: 5, qrEnSize: 3.4,
  termSize: 3.1, termLineH: 4, termGap: 1.1,
  footerH: 12
};
BACK.benefitsTextX = MARGIN + BACK.benefitIcon + 5;
BACK.benefitsW = W - BACK.benefitsTextX - MARGIN;
BACK.qrTextX = MARGIN + BACK.qrBox + 7;
BACK.qrTextW = W - BACK.qrTextX - MARGIN;
BACK.termsTextX = MARGIN + 6;
...
function formatJoinDate(createdAt) {
  const raw = String(createdAt || '').split(' ')[0];
...
function measureFieldsHeight(doc, fields) {
  ...
  return fields.reduce((sum, f) => {
    const valueH = wrappedHeight(doc, `: ${f.value}`, FRONT.valueW, valueOpts);
    return sum + Math.max(FRONT.valueLineH, valueH) + FRONT.rowGap;
  }, 0);
}
...
async function streamCardPdf(res, member, qrBuffer) {
  ...
  doc.rect(0, 0, PAGE_W, H).fill(WHITE);
...
function drawFront(doc, member, H) {
  doc.save();
  doc.roundedRect(0.5, 0.5, W - 1, H - 1, CARD_RADIUS).clip();
  ...
  doc.roundedRect(MARGIN, badgeY, W - 2 * MARGIN, FRONT.badgeH, 3).fill(GREEN);
  centeredMixedText(doc, 'DIGITAL MEMBERSHIP CARD', W / 2, badgeY + 2.9, { kannadaFont: 'KN-Bold', latinFont: 'Helvetica-Bold', fontSize: 4.8, color: WHITE });
  ...
  if (photoPath && fs.existsSync(photoPath)) {
    try {
      doc.save();
      doc.roundedRect(MARGIN, bodyY, FRONT.photoW, FRONT.photoH, 3).clip();
      doc.image(photoPath, MARGIN, bodyY, { width: FRONT.photoW, height: FRONT.photoH });
      doc.restore();
    } catch (e) { /* ignore malformed image */ }
  } else {
    doc.roundedRect(MARGIN, bodyY, FRONT.photoW, FRONT.photoH, 3).fill('#eef0f5');
    doc.fillColor('#9aa3b5').font('Helvetica').fontSize(4).text('No Photo', MARGIN, bodyY + FRONT.photoH / 2 - 2, { width: FRONT.photoW, align: 'center' });
  }
  doc.roundedRect(MARGIN, bodyY, FRONT.photoW, FRONT.photoH, 3).lineWidth(0.8).stroke(GOLD);

  const joinDate = formatJoinDate(member.created_at);
  const fields = frontFields(member, joinDate);
  const labelOpts = { kannadaFont: 'KN-Regular', latinFont: 'Helvetica', fontSize: FRONT.labelSize, color: MUTED };
  const valueOpts = { kannadaFont: 'KN-Bold', latinFont: 'Helvetica-Bold', fontSize: FRONT.valueSize, lineHeight: FRONT.valueLineH };
  ...
  fields.forEach((f) => {
    mixedText(doc, f.label, FRONT.fieldsX, fy + 0.6, labelOpts);
    const valueText = `: ${f.value}`;
    const valueH = drawWrapped(doc, valueText, FRONT.valueX, fy, FRONT.valueW, { ...valueOpts, color: f.id ? GREEN : BLUE });
    fy += Math.max(FRONT.valueLineH, valueH) + FRONT.rowGap;
  });

  const bodyBottom = Math.max(bodyY + FRONT.photoH, fy);
  const signRowH = computeSignRowHeight(doc);
  const signY = Math.max(bodyBottom + FRONT.sectionGap, H - FRONT.footerH - FRONT.waveH - signRowH - 3);
  ...
  doc.save();
  doc.dash(1.1, { space: 1.5 }).lineWidth(0.7).strokeColor(GREEN);
  doc.circle(sealCx, sealCy, sealR - 0.5).stroke();
  doc.undash();
  doc.restore();
  doc.circle(sealCx, sealCy, sealR * 0.42).fill(GREEN);
  drawCheckmark(doc, sealCx, sealCy, sealR * 0.5, WHITE);

  const capTopY = signY + FRONT.signImgSize + 1.2;
  doc.moveTo(MARGIN, capTopY).lineTo(MARGIN + FRONT.signCapW, capTopY).strokeColor(LINE).lineWidth(0.4).stroke();
  ...
  doc.moveTo(FRONT.signDividerX, signY).lineTo(FRONT.signDividerX, signY + signRowH).strokeColor(LINE).lineWidth(0.4).stroke();

  const verifyBlockH = VERIFY_LINES.length * FRONT.verifyLineH;
  let vy = signY + (signRowH - verifyBlockH) / 2;
  VERIFY_LINES.forEach((line) => {
    mixedText(doc, line, FRONT.verifyX, vy, { kannadaFont: 'KN-Bold', latinFont: 'Helvetica-Bold', fontSize: FRONT.verifySize, color: GOLD_DEEP });
  ...
  drawIcon(doc, iconGlobe, MARGIN, footMidY - 3, 6, WHITE);
  doc.fillColor(WHITE).font('Helvetica-Bold').fontSize(4.4).text('www.rjpindia.com', MARGIN + 8, footMidY - 2);

  const chip = 7.5, iconSize = 4.2, gap = 2.8;
  ...
  doc.restore();
  doc.roundedRect(0.5, 0.5, W - 1, H - 1, CARD_RADIUS).lineWidth(0.5).strokeColor('#D7DBE3').stroke();
}
...
function drawBack(doc, qrBuffer, H) {
  ...
  doc.circle(0, 0, 15).fillOpacity(0.08).fill(WHITE).fillOpacity(1);
  doc.restore();

  centeredMixedText(doc, 'ರಾಷ್ಟ್ರೀಯ ಜನಹಿತ ಪಕ್ಷ (RJP)', W / 2, 7.5, { kannadaFont: 'KN-Bold', latinFont: 'Helvetica-Bold', fontSize: 5.8, color: GOLD });
  centeredMixedText(doc, 'RASHTRIYA JANAHITA PARTY', W / 2, 15, { kannadaFont: 'KN-Regular', latinFont: 'Helvetica-Bold', fontSize: 3.6, color: WHITE });
  centeredMixedText(doc, 'ಜನ ಮೊದಲು... ಅಧಿಕಾರ ನಂತರ', W / 2, 21.2, { kannadaFont: 'KN-Regular', latinFont: 'Helvetica', fontSize: 3.7, color: '#dff3e6' });
  ...
  BENEFITS.forEach((t, i) => {
    ...
    const h = drawWrapped(doc, t, BACK.benefitsTextX, y + (BACK.benefitIcon - BACK.benefitLineH) / 2, BACK.benefitsW, { ...benefitOpts, color: '#1a1a1a' });
    y += Math.max(h, BACK.benefitIcon) + (i < BENEFITS.length - 1 ? BACK.benefitGap : 0);
  });
  y += BACK.sectionGap;

  doc.moveTo(MARGIN, y - BACK.sectionGap / 2).lineTo(W - MARGIN, y - BACK.sectionGap / 2).strokeColor(LINE).lineWidth(0.4).stroke();

  const qrKnOpts = { kannadaFont: 'KN-Bold', latinFont: 'Helvetica-Bold', fontSize: BACK.qrKnSize, lineHeight: BACK.qrKnLineH };
  const qrKnH = wrappedHeight(doc, 'ನಿಮ್ಮ ಸದಸ್ಯತ್ವವನ್ನು ಪರಿಶೀಲಿಸಲು QR ಕೋಡ್ ಸ್ಕ್ಯಾನ್ ಮಾಡಿ', BACK.qrTextW, qrKnOpts);
  const qrTextH = qrKnH + 2.5 + BACK.qrEnSize;
  const qrRowH = Math.max(BACK.qrBox, qrTextH);

  doc.roundedRect(MARGIN, y, BACK.qrBox, BACK.qrBox, 3).fill(WHITE);
  doc.roundedRect(MARGIN, y, BACK.qrBox, BACK.qrBox, 3).lineWidth(0.8).stroke(GREEN);
  if (qrBuffer) {
    try { doc.image(qrBuffer, MARGIN + 2.2, y + 2.2, { width: BACK.qrBox - 4.4, height: BACK.qrBox - 4.4 }); } catch (e) { /* ignore */ }
  }
  ...
  mixedText(doc, 'Scan QR to Verify Your Membership', BACK.qrTextX, qrTextY + qrKnH + 2.5, { kannadaFont: 'KN-Regular', latinFont: 'Helvetica-Bold', fontSize: BACK.qrEnSize, color: GREEN_DEEP });

  y += qrRowH + BACK.sectionGap;

  const termOpts = { kannadaFont: 'KN-Regular', latinFont: 'Helvetica', fontSize: BACK.termSize, lineHeight: BACK.termLineH };
  TERMS.forEach((t, i) => {
    doc.circle(MARGIN + 2.8, y + 2.4, 2.8).fill(GREEN);
    drawCheckmark(doc, MARGIN + 2.8, y + 2.4, 2.2, WHITE);
    const h = drawWrapped(doc, t, BACK.termsTextX, y, BACK.termsW, { ...termOpts, color: '#3a4152' });
    y += h + (i < TERMS.length - 1 ? BACK.termGap : 0);
  });

  doc.rect(0, H - BACK.footerH, W, BACK.footerH).fill(GREEN_DEEP);
  doc.fillColor('#dff3e6').font('Helvetica').fontSize(3.3).text('RJP HELPLINE', MARGIN, H - BACK.footerH + 2.8);
  doc.fillColor(GOLD).font('Helvetica-Bold').fontSize(5.7).text('9632527961', MARGIN, H - BACK.footerH + 6.8);

  const footMidY = H - BACK.footerH / 2;
  const emailText = 'info@rjpindia.com';
  doc.font('Helvetica').fontSize(3.8);
  const emailW = doc.widthOfString(emailText);
  const chip = 6.5, iconSize = 3.4, sgap = 2.4;
  const socialW = chip * SOCIAL_ICONS.length + sgap * (SOCIAL_ICONS.length - 1);
  const rightGroupW = emailW + 5 + socialW;
  const emailX = W - MARGIN - rightGroupW;
  doc.fillColor(WHITE).text(emailText, emailX, footMidY - 1.8);
  drawSocialRow(doc, SOCIAL_ICONS, emailX + emailW + 5, footMidY, { chip, iconSize, gap: sgap, color: WHITE, chipOpacity: 0.18 });
