...
function drawFront(doc, member) {
  ...
  const headerGrad = doc.linearGradient(0, 0, W, 160);
  headerGrad.stop(0, '#FBDD86').stop(0.45, GOLD).stop(1, GREEN);
  doc.save();
  doc.moveTo(0, 0)
    .lineTo(W, 0)
    .lineTo(W, 128)
    .bezierCurveTo(W * 0.78, 168, W * 0.58, 118, W * 0.36, 142)
    .bezierCurveTo(W * 0.18, 162, W * 0.08, 172, 0, 148)
    .closePath()
    .clip();
  doc.rect(0, 0, W, 175).fill(headerGrad);
  doc.restore();
  ...
  const fields = [
    ['ಸದಸ್ಯ ಹೆಸರು', member.full_name],
    ['ಸದಸ್ಯ ID', member.application_number],
    ['ಜಿಲ್ಲೆ', member.district || '-'],
    ['ವಿಧಾನಸಭಾ ಕ್ಷೇತ್ರ', member.assembly || '-'],
    ['ಸದಸ್ಯತ್ವ ದಿನಾಂಕ', (member.created_at || '').split(' ')[0]]
  ];
  let fy = 192;
  const fx = 120;
  fields.forEach(([label, value]) => {
    mixedText(doc, label, fx, fy, { kannadaFont: 'KN-Regular', latinFont: 'Helvetica', fontSize: 7.5, color: '#6b7280' });
    mixedText(doc, String(value), fx, fy + 11, { kannadaFont: 'KN-Bold', latinFont: 'Helvetica-Bold', fontSize: 9.5, color: NAVY });
    fy += 27;
  });

  doc.moveTo(20, 418).lineTo(110, 418).strokeColor('#c7cbd6').stroke();
  doc.fillColor('#6b7280').font('Helvetica').fontSize(7).text('Authorised Signatory', 20, 422, { width: 90, align: 'center' });

  doc.circle(150, 405, 18).fill(GREEN);
  drawCheckmark(doc, 150, 405, 18, WHITE);
  mixedText(doc, 'ಹೊಸ ಚಿಂತನೆ...', 178, 392, { kannadaFont: 'KN-Bold', latinFont: 'Helvetica-Bold', fontSize: 7.5, color: GOLD });
  mixedText(doc, 'ಹೊಸ ರಾಜಕೀಯ...', 178, 404, { kannadaFont: 'KN-Bold', latinFont: 'Helvetica-Bold', fontSize: 7.5, color: GOLD });
  mixedText(doc, 'ಹೊಸ ಭವಿಷ್ಯ...', 178, 416, { kannadaFont: 'KN-Bold', latinFont: 'Helvetica-Bold', fontSize: 7.5, color: GOLD });
