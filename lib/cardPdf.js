...
const SEAL_IMG = path.join(__dirname, '..', 'assets', 'images', 'card-verify-seal.png');
...
const FRONT = {
  headerH: 170, badgeGap: 8, badgeH: 20, bodyGap: 12,
...
async function streamCardPdf(res, member, qrBuffer) {
  ...
  const pageW = PAGE_W + PAGE_MARGIN * 2;
  const pageH = H + PAGE_MARGIN * 2;
  ...
  doc.rect(0, 0, pageW, pageH).fill('#EDEFF3');
  ...
  doc.save();
  doc.translate(PAGE_MARGIN + W + GAP, PAGE_MARGIN);
...
function drawFront(doc, member, H) {
  ...
  if (fs.existsSync(SEAL_IMG)) {
    try {
      doc.image(SEAL_IMG, imgX, signY, {
        fit: [FRONT.signImgSize, FRONT.signImgSize], align: 'center', valign: 'center'
      });
    } catch (e) { /* ignore malformed image */ }
  }
