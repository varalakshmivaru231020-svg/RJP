...
function getCmsSection(section) {
  const defaults = CMS_DEFAULTS[section] || {};
  const row = db.prepare('SELECT data FROM cms_content WHERE section = ?').get(section);
...
function saveCmsSection(section, data) {
  db.prepare(`
    INSERT INTO cms_content (section, data, updated_at) VALUES (?, ?, datetime('now'))
