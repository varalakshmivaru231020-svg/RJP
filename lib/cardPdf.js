...
const SEAL_IMG = path.join(__dirname, '..', 'assets', 'images', 'card-verify-seal.jpg');
...
const SIGN_CAPTION = 'ಅಧಿಕೃತ ಸಹಿ / Authorised Signatory';
...
function computeSignRowHeight(doc) {
  const capOpts = { kannadaFont: 'KN-Regular', latinFont: 'Helvetica', fontSize: FRONT.capSize, lineHeight: FRONT.capLineH };
...
function drawFront(doc, member, H) {
  ...
  if (fs.existsSync(SEAL_IMG)) {
    try { doc.image(SEAL_IMG, imgX, signY, { width: FRONT.signImgSize, height: FRONT.signImgSize }); } catch (e) { /* ignore malformed image */ }
  }
  ...
  centeredWrapped(doc, SIGN_CAPTION, MARGIN + FRONT.signCapW / 2, capTopY + FRONT.capGap, FRONT.signCapW, { kannadaFont: 'KN-Regular', latinFont: 'Helvetica', fontSize: FRONT.capSize, lineHeight: FRONT.capLineH, color: MUTED });
